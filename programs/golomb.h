...
class Golomb {
    ...
    public:
        ...
        std::vector<int> decode(std::string encoded_string) {
            calculateBits(m);
            ...
            while ((size_t)i < encoded_string.length()) {
                int quotient = 0;
                ...
                if (m != 1) {
                    ...
                    // convert the temporary string to an integer to get the remainder
                ...
                // if the encoded string has a 1 at the end of the remainder, the result is negative, otherwise it's positive
